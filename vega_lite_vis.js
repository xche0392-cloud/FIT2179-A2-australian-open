vegaEmbed("#attendance_chart", "js/attendance_chart.vg.json").then(function(result) {
}).catch(console.error);

vegaEmbed("#womens_champions_chart", "js/womens_champions_chart.vg.json").then(function(result) {
}).catch(console.error);

vegaEmbed("#mens_champions_chart", "js/mens_champions_chart.vg.json").then(function(result) {
}).catch(console.error);

vegaEmbed("#top_nations_chart", "js/top_nations_chart.vg.json").then(function(result) {
}).catch(console.error);

vegaEmbed("#aus_results_chart", "js/aus_results_chart.vg.json").then(function(result) {
}).catch(console.error);

vegaEmbed("#prize_chart", "js/prize_chart.vg.json").then(function(result) {
}).catch(console.error);

vegaEmbed("#titles_by_country_chart", "js/titles_by_country_chart.vg.json").then(function(result) {
}).catch(console.error);

vegaEmbed("#legend_heatmap", "js/legend_heatmap.vg.json").then(function(result) {
}).catch(console.error);

vegaEmbed("#age_winrate_chart", "js/age_winrate_chart.vg.json").then(function(result) {
}).catch(console.error);

vegaEmbed("#world_map", "js/world_map.vg.json").then(function(result) {
}).catch(console.error);

vegaEmbed("#aus_states_map", "js/aus_states_map.vg.json").then(function(result) {
}).catch(console.error);